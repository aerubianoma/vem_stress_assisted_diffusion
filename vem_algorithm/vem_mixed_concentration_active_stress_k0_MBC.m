function [uh,ph,info] = vem_mixed_concentration_active_stress_k0_MBC(node,elem,pde,bdStruct,phi0,phi1,info_diffusion)
...
for iel = 1:NT
    ...
    if  ~isa(info_diffusion,'struct')
        ...
        c_phi = phi_previous(iel);

    end
    

    l_l = l_local_k_0(phi0,phi1,c_phi);
    l = l_l.g;
...
if ~isempty(bdEdgeN)
    ...
    FN1 = 1/6*(sum(pm1(:,1:2).*Ne,2)-pe(z1).*Ne(:,1));  
    FN2 = 1/6*(sum(pm2(:,1:2).*Ne,2)-pe(z2).*Ne(:,1));
    FN3 = 4/6*(sum(pm3(:,1:2).*Ne,2)-pe(ze).*Ne(:,1));
    FN4 = 1/6*(sum(pm1(:,3:4).*Ne,2)-pe(z1).*Ne(:,2));  
    FN5 = 1/6*(sum(pm2(:,3:4).*Ne,2)-pe(z2).*Ne(:,2));
    FN6 = 4/6*(sum(pm3(:,3:4).*Ne,2)-pe(ze).*Ne(:,2));
