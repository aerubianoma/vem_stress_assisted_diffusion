function uH1 = error_V1_battery(node,elem,uh,info,pde)
    ...
    for s = 1:Nv  % loop over triangles of each element
        ...
        for p = 1:length(weight)
            ...
            if size(a,2)<=Nm  % scalar case
                ...
                amx = [sum(aNv(:,1:Nm).*mx,2), 0.5*(sum(aNv(:,1:Nm).*my,2)+sum(aNv(:,Nm+1:end).*mx,2))]; % [am1, am2]
                amy = [0.5*(sum(aNv(:,1:Nm).*my,2)+sum(aNv(:,Nm+1:end).*mx,2)), sum(aNv(:,Nm+1:end).*my,2)];
                %Eu = Eu(:,[1,3,2,4]); % [u1x,u1y,u2x,u2y] --> [u1x,u2x,u1y,u2y]
            end
            agradm = [amx, amy];
            %err = err + weight(p)*sum((Eu-agradm).^2,2);
            u = u + weight(p)*sum(agradm.^2,2);
        end
    end
    %ErrH1(idNv) = area.*sum(err,2);
    uH1(idNv) = area.*sum(u,2);
end
%ErrH1 = sum(ErrH1); % total errors
uH1 = sum(uH1);
h = mean(aux.diameter);
% Weighted norm
%ErrH1 = sqrt(2*pde.mu*ErrH1);
uH1 = sqrt(pde.mu*uH1);
