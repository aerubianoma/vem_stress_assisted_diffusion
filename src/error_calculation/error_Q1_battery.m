function pL2 = error_Q1_battery(node,elem,ph,pde)
...
h = mean(aux.diameter);
% Weighted error
%ErrpL2 = sqrt((1/(2*pde.mu))*ErrpL2+(1/(pde.lambda)).^2*ErrpL2); %sqrt(ErrpL2)/sqrt(pL2);
pL2 = sqrt((1/(pde.mu))*pL2+(1/pde.lambda).^2*pL2);
