...
for k = 1:maxIt
    ...
    while true
        ...
        [uh,ph,info_stress_next] = vem_mixed_concentration_active_stress_k1_MBC(node,elem,data_stress,bdStruct,phi0,phi1,info_diffusion);
        
        %% k_2 = 0 for transport mixed formulation (mixed boundary conditions) 
        
        [zetah,phih,info_diffusion_next] = vem_mixed_stress_assisted_diffusion_k1_MBC(node,elem,data_diffusion,bdStruct,m0,m1,info_stress_next);
        ...
        [Err_zeta_it,Err_phi_it,zeta_it,phi_it] = error_V2_Q2_k1(node,elem,zetah,phih,info_diffusion_next,data_diffusion,M);
        ...
        if size(it,2) > 1
            ...
            [Err_p_stop,p_stop] = error_Q1(node,elem,ph-ph_prev,data_stress);
            [Err_zeta_stop,Err_phi_stop,zeta_stop,phi_stop] = error_V2_Q2_k1(node,elem,zetah-zetah_prev,phih-phih_prev,info_diffusion_next,data_diffusion,M);
    ...
    %saveas(gcf, fullfile('fancy_plots/k21', u_file));
    ...
    %saveas(gcf, fullfile('fancy_plots/k21', zeta_file));
    ...
    %saveas(gcf, fullfile('fancy_plots/k21', p_phi_file));
...
%saveas(gcf, fullfile('fancy_plots/k21', error_convergence_file));
